...
class myResnet(nn.Module):
    def __init__(self, in_channels=3, num_out=10):
        ...
        self.cnn_layer2 = nn.Sequential(nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding=1),
                                        nn.MaxPool2d(kernel_size=2, stride=2),
                                        nn.BatchNorm2d(64),
                                        nn.ReLU())
        self.layer3 = residual_block(64)
        self.cnn_layer4 = nn.Sequential(nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, padding=1),
                                        nn.MaxPool2d(kernel_size=2, stride=2),
                                        nn.BatchNorm2d(128),
                                        nn.ReLU())
        self.layer5 = residual_block(128)
        self.fc1 = nn.Sequential(nn.Linear(8192, 512), nn.ReLU())
        self.fc2 = nn.Sequential(nn.Linear(512, num_out))
