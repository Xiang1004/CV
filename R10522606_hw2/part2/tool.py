...
def train(model, train_loader, val_loader, num_epoch, log_path, save_path, device, criterion, scheduler, optimizer):
    ...
    for i in range(num_epoch):
        ...
        for batch_idx, (data, label,) in enumerate(tqdm(train_loader)):
            ...
            # if the gradient is too large, we don't adopt it
        ...
        sec = elp_time % 60
        print('*'*10)
        print('time = {:.4f} MIN {:.4f} SEC, total time = {:.4f} Min {:.4f} SEC '.format(minu, sec, (end_time-start_train) // 60, (end_time-start_train) % 60))
        ...
        with open(log_path, 'a') as f:
            f.write(f'epoch = {i}\n', )
            f.write('time = {:.4f} MIN {:.4f} SEC, total time = {:.4f} Min {:.4f} SEC\n'.format(minu, sec, (end_time-start_train) // 60, (end_time-start_train) % 60))
    ...
    plot_learning_curve(x, overall_loss, 'train_loss')
    plot_learning_curve(x, overall_val_loss, 'val_loss')
    plot_learning_curve(x, overall_acc, 'train_acc')
    plot_learning_curve(x, overall_val_acc, 'val_acc')
