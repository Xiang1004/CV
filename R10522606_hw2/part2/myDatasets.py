...
def get_cifar10_train_val_set(root, ratio=0.9):

    # get all the images path and the corresponding labels
    with open(root, 'r') as f:
        data = json.load(f)
    images_ori, labels_ori = data['images'], data['categories']
    images, labels = [], []
    for i in range(len(images_ori)):
        if len(images_ori[i]) < 11:
            images.append(images_ori[i])
    ...
    for i in range(len(all_images)):
        labels.append(int(all_images[i][0]))
    ...
    # normally, we don't apply transform to test_set or val_set
...
class cifar10_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        path = os.path.join(self.prefix, image_name)
