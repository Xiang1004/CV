...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    all_dis = distance.cdist(test_image_feats, train_image_feats, metric='cityblock')
    # all_dis = distance.cdist(test_image_feats, train_image_feats, metric='euclidean')
    for dis in all_dis:
        knn = np.argpartition(dis, k)[:k]       # Compared with the k-th largest number
        test_predicts.append(mode(label[knn])[0][0])  # count the most
        # knn = sorted(dis)[:k] / max(knn, knn.conut)  --can't sort array
