...
def build_vocabulary(image_paths, vocab_size):
    ...
    bag_of_features = []
    for path in image_paths:            # scan all pictures
        img = cv2.imread(path, 0)
        _, descriptors = dsift(img, step=[1, 1], fast=True)
        i = 0
        for des in descriptors:
            if i % 2 == 0:              # only trying to get a representative sample here.
                bag_of_features.append(des)
            i += 1

    vocab = kmeans(np.matrix(bag_of_features).astype('float32'), vocab_size)
